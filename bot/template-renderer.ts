...
export function renderSite(data: SiteData): string {
  ...
  // Inject LocalBusiness JSON-LD for SEO (all sites)
  const jsonLd = {
    ...
    url: `https://whatswebsite.com/site/${data.slug}`,
  ...
  if (data.plan !== 'premium') {
    ...
    <div style="margin:16px auto;max-width:728px;text-align:center;overflow:hidden;">
      <ins class="adsbygoogle" style="${style}" data-ad-client="${ADSENSE_PUB_ID}" data-ad-format="${format}" data-full-width-responsive="true"></ins>
